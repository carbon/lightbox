...
(function (Carbon) {
    document.addEventListener('keydown', e => {
        if (!window.lightbox)
            return;
        if (!window.lightbox.element.classList.contains('open'))
            return false;
        switch (e.keyCode) {
            case 39:
...
`;
    class Lightbox {
        ...
        onPanEnd(e) {
            ...
            this.didPan = true;
            if (a < -0.3 && this.panDirection === 2 && this.nextSlide) {
                this.next();
                return;
            }
            else if (a > 0.3 && this.panDirection === 4 && this.prevSlide) {
                this.prev();
                return;
            }
        ...
        buildSlide(item) {
            ...
            slide.objectEl.removeAttribute('srcset');
            slide.objectEl.removeAttribute('data-src');
            slide.objectEl.removeAttribute('data-srcset');
            slide.fitObject();
            return slide;
        }
        fit(element, box) {
